...
def verify_account(request: HttpRequest):
    if request.method == "POST":
        ...
        if pending_user and pending_user.is_valid():
            ...
            messages.success(
                request, "Account verified. You are now logged in")
    ...
    if request.method == "POST":
        ...
        if user:
            token, _ = Token.objects.update_or_create(
                user=user, token_type=TokenType.PASSWORD_RESET,
                defaults={
            ...
            messages.success(request, "Reset link to your email")
...
def verify_password_reset_link(request: HttpRequest):
    ...
    token: Token = Token.objects.filter(
        user__email=email, token=reset_token, token_type=TokenType.PASSWORD_RESET).first()
